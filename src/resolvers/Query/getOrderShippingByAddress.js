...
export default async function getOrderShippingByAddress(parent, { input } = {}, context) {
  console.log("input", input);
  const { address, city, amount } = input;
  // if (!address || !city) {
  //   throw new ReactionError("invalid-param", "You must provide address and city arguments");
  ...
  if (ShippingMethodsObj) {
    const ShippingMethods = ShippingMethodsObj[0];
    let selectedFulfillmentId = null;
    const selectedMethodId = ShippingMethods?.methods.filter(item => item.group.toLowerCase() == "ground" && item.enabled);
    if (selectedMethodId.length > 0) {
      selectedFulfillmentId = await encodeFulfillmentGroupOpaqueId(selectedMethodId[0]?._id);

      return {
        ...selectedMethodId[0],
        // _id:selectedFulfillmentId
      };
    }
    throw new ReactionError("", "No suitable fulfillment method found, please check the fulfillment methods.");
  } else {
    throw new ReactionError("", "No Fulfillment methods found, create a fulfillment method and try again");
  }
}
